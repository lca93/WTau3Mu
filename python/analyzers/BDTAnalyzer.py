...
class BDTAnalyzer(Analyzer):
    ...
    def process(self, event):
        ...
        event_variables = np.array([
            ...
            vtx.ls                                                  ,
            vtx.cos                                                 ,
            cand.mu1().relIso(cone_size=0.4, iso_type='dbeta', dbeta_factor=0.5,  all_charged=0),
            cand.mu2().relIso(cone_size=0.4, iso_type='dbeta', dbeta_factor=0.5,  all_charged=0),
            cand.mu3().relIso(cone_size=0.4, iso_type='dbeta', dbeta_factor=0.5,  all_charged=0),
            cand.met().pt()                                         ,
            int(mu1ID)                                              ,
            int(mu2ID)                                              ,
            int(mu3ID)                                              ,
